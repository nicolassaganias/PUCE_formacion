...
#define CALIBRATION_SAMPLE_INTERVAL (500)  //define the time interval(in millisecond) between each samples in the \
                                           //calibration phase
#define READ_SAMPLE_INTERVAL (50)          //define the time interval(in millisecond) between each samples in
...
void setup() {
  ...
  lcd.print(Ro);
  lcd.print("kohm");
  delay(2000);  // Show calibration result for 2 seconds
  lcd.clear();
}
...
void loop() {
  ...
  // For more display options (if you want to show all three gases):
  // You might need to scroll or cycle through displays since 16x2 LCD has limited space
